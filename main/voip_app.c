...
static int _sip_event_handler(sip_event_msg_t *event)
{
    ...
    switch ((int)event->type) {
        ...
        case SIP_EVENT_RINGING:
            ...
            esp_sip_uas_answer(sip, true);  // Atende automaticamente
            break;
...
static esp_err_t input_key_service_cb(periph_service_handle_t handle, periph_service_event_t *evt, void *ctx)
{
    ...
    if (evt->type == INPUT_KEY_SERVICE_ACTION_CLICK_RELEASE) {
        ...
        if (sip_state < SIP_STATE_REGISTERED) {
            ESP_LOGW(TAG, "SIP not registered, cannot make call."); // Adicionado log de aviso
            return ESP_OK;
        }
        switch ((int)evt->data) {
            case INPUT_KEY_USER_ID_REC:
            case INPUT_KEY_USER_ID_PLAY: // <--- Botão de Play/Pause (INPUT_KEY_USER_ID_PLAY)
                ESP_LOGI(TAG, "[ * ] [Play] input key event - Initiating Call to %s", DESTINATION_SIP_URI); // Log mais descritivo
                // Verifica se já não está em uma chamada para evitar discagens múltiplas
                if (!(sip_state & SIP_STATE_ON_CALL) && !(sip_state & SIP_STATE_CALLING) && !(sip_state & SIP_STATE_SESS_PROGRESS)) {
                    esp_sip_uac_invite(sip, DESTINATION_SIP_URI); // Faz a chamada SIP
                } else {
                    ESP_LOGW(TAG, "Already in a call state, ignoring call attempt.");
                }
                break;
            ...
            case INPUT_KEY_USER_ID_SET:
                ...
                if (sip_state & SIP_STATE_RINGING) {
                    esp_sip_uas_answer(sip, false); // Decline call if ringing
                } else if (sip_state & SIP_STATE_ON_CALL) {
                    esp_sip_uac_bye(sip); // Hangup if on call
                } else if ((sip_state & SIP_STATE_CALLING) || (sip_state & SIP_STATE_SESS_PROGRESS)) {
                    esp_sip_uac_cancel(sip); // Cancel if calling or session in progress
                }
                break;
